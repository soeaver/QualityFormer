...
class QEMLossComputation(object):
    def __init__(self, cfg):
        self.device = torch.device(cfg.DEVICE)
        self.num_parsing = cfg.PARSING.NUM_PARSING
        self.parsing_loss_weight = cfg.PARSING.QEM.PARSING_LOSS_WEIGHT
        self.lovasz_loss_weight = cfg.PARSING.QEM.LOVASZ_LOSS_WEIGHT
        self.iou_loss_weight = cfg.PARSING.QEM.IOU_LOSS_WEIGHT
    ...
    def __call__(self, parsing_logits, iou_pred, parsing_targets):
        ...
        return dict(loss_qem_parsing=parsing_loss, loss_qem_iou=iou_loss)


def qem_loss_evaluator(cfg):
    loss_evaluator = QEMLossComputation(cfg)
